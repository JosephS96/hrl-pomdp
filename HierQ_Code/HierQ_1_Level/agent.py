...
class Agent():
    ...
    def update_critic(self,old_state,action,reward,next_state,goal,done):
        ...
        if not done:
            # next_action = np.argmax(self.critic[next_state])
            target = reward + gamma * np.amax(self.critic[next_state])
        else:
            target = reward

        # print("Target: ", target)
        # print("Action: ", action)
        # print("Old Q(s,a): ", self.critic[old_state][action])

        # Push Q(s,a) closer to target 
        self.critic[old_state][action] = self.critic[old_state][action] + step_size * (target - self.critic[old_state][action])

        # print("New Q(s,a): ", self.critic[old_state][action])
