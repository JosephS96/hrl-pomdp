...
class Agent():
    ...
    def __init__(self,num_squares,num_actions,time_limits,FLAGS):
        if FLAGS.retrain:
            ...
            self.critic_lay2 = np.ones((num_squares,num_squares)) * -num_squares
    ...
    def get_action(self,state,FLAGS,layer,goal = None):
        if FLAGS.test or np.random.random_sample() > epsilon:
            ...
            if layer == 2:
                action = rand_argmax(self.critic_lay2[state],goal)
            elif layer == 1:
                action = rand_argmax(self.critic_lay1[goal][state],goal)
            elif layer == 0:
    ...
    def update_critic_lay1(self,old_state,action,reward,next_state,goal,done):
        ...
        # Determine target from Bellman equation
        if not done:
            target = reward + gamma * np.amax(self.critic_lay1[goal][next_state])
        ...
        # Push Q(s,a) closer to target 
        self.critic_lay1[goal][old_state][action] = self.critic_lay1[goal][old_state][action] + step_size * (target - self.critic_lay1[goal][old_state][action])

        # print("New Q1(s,a): ", self.critic_lay1[old_state][action])

    def update_critic_lay2(self,old_state,action,reward,next_state,goal,done):
        ...
        # Determine target from Bellman equation
        if not done:
            target = reward + gamma * np.amax(self.critic_lay2[next_state])
            # next_action = np.argmax(self.critic_lay1[next_state])
            # target = reward + gamma * self.critic_lay1[next_state][next_action]
        else:
            target = reward
        ...
        # Push Q(s,a) closer to target 
        self.critic_lay2[old_state][action] = self.critic_lay2[old_state][action] + step_size * (target - self.critic_lay2[old_state][action])
